<?php

?>
<!DOCTYPE html>
<html>
    <head>
        <title>MRTenable</title>
        <link rel="stylesheet" href="/css/index.css">
    </head>
    <div id="main_container">
        <h1 id="header_text">Tenable Answers</title>
        <h2 id="question">Question: </h2>
        <div id="answers_container">
            <div id="answer_container_1" class="answer_container">
                <p id="answer_1" class="answer"></p>
            </div>
            <div id="answer_container_2" class="answer_container">
                <p id="answer_2" class="answer"></p>
            </div>
            <div id="answer_container_3" class="answer_container">
                <p id="answer_3" class="answer"></p>
            </div>
            <div id="answer_container_4" class="answer_container">
                <p id="answer_4" class="answer"></p>
            </div>
            <div id="answer_container_5" class="answer_container">
                <p id="answer_5" class="answer"></p>
            </div>
            <div id="answer_container_6" class="answer_container">
                <p id="answer_6" class="answer"></p>
            </div>
            <div id="answer_container_7" class="answer_container">
                <p id="answer_7" class="answer"></p>
            </div>
            <div id="answer_container_8" class="answer_container">
                <p id="answer_8" class="answer"></p>
            </div>
            <div id="answer_container_9" class="answer_container">
                <p id="answer_9" class="answer"></p>
            </div>
            <div id="answer_container_10" class="answer_container">
                <p id="answer_10" class="answer"></p>
            </div>
        </div>
    </div>
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js"></script>
    <script src="/js/index.js"></script>
</html>